...
<template>
  <div class="min-h-screen bg-[#060a0c] pt-20 relative overflow-hidden">
    ...
    <!-- Main split layout -->
    <div class="relative z-10 max-w-5xl mx-auto px-4 py-10 flex flex-col lg:flex-row gap-0 min-h-[80vh]">

      <!-- LEFT: Celebration panel -->
      <div class="lg:w-[42%] rounded-t-3xl lg:rounded-l-3xl lg:rounded-tr-none bg-gradient-to-br from-emerald-900 via-teal-800 to-emerald-950 p-10 flex flex-col items-center justify-center text-center relative overflow-hidden">
        <!-- Decorative rings -->
        <div class="absolute -top-20 -left-20 w-60 h-60 border border-emerald-500/20 rounded-full" />
        <div class="absolute -bottom-16 -right-16 w-48 h-48 border border-teal-400/15 rounded-full" />
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 border border-emerald-400/10 rounded-full animate-pulse-glow" />

        <!-- Success icon -->
        <div class="relative mb-8">
          <div class="w-28 h-28 rounded-full bg-emerald-500/20 flex items-center justify-center animate-pop">
            <div class="w-20 h-20 rounded-full bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center shadow-lg shadow-emerald-500/40">
        ...
        <!-- Order ID badge -->
        <div class="inline-flex items-center gap-2 bg-white/10 backdrop-blur rounded-full px-5 py-2">
      ...
      <!-- RIGHT: Details panel -->
      <div class="lg:w-[58%] rounded-b-3xl lg:rounded-r-3xl lg:rounded-bl-none bg-[#10101c] border border-white/5 p-8 sm:p-10 flex flex-col justify-between">

        <!-- License key card -->
        <div class="mb-8">
          <div class="text-xs text-emerald-400 font-bold uppercase tracking-widest mb-3 flex items-center gap-2">
          ...
          <div class="bg-[#060a0c] rounded-2xl p-5 flex items-center justify-between gap-3 border border-emerald-500/20">
            <span class="font-mono text-emerald-400 text-lg font-bold tracking-wider select-all">{{ licenseKey }}</span>
            <button
              @click="copyKey"
              class="shrink-0 px-4 py-2 rounded-lg text-xs font-bold transition-all duration-300 cursor-pointer"
              :class="copied ? 'bg-emerald-500 text-white' : 'bg-white/10 text-gray-300 hover:bg-white/20'"
            >
              {{ copied ? '✓ Copied!' : 'Copy' }}
            </button>
          </div>
        ...
        <div class="mb-8 space-y-0">
          <div class="text-xs text-gray-500 font-semibold uppercase tracking-widest mb-4">Detail Pembelian</div>

          <div class="relative pl-6 border-l-2 border-emerald-500/30 space-y-5">
            <!-- Buyer -->
            <div class="relative">
              <div class="absolute -left-[29px] top-1 w-3 h-3 rounded-full bg-emerald-500 ring-4 ring-[#10101c]" />
              <p class="text-gray-500 text-xs mb-0.5">Pembeli</p>
              <p class="text-white text-sm font-medium">{{ buyerName }}</p>
            </div>
            <!-- Email -->
            <div class="relative">
              <div class="absolute -left-[29px] top-1 w-3 h-3 rounded-full bg-teal-500 ring-4 ring-[#10101c]" />
              <p class="text-gray-500 text-xs mb-0.5">Email</p>
              <p class="text-white text-sm font-medium">{{ buyerEmail }}</p>
            </div>
            <!-- Product -->
            <div class="relative">
              <div class="absolute -left-[29px] top-1 w-3 h-3 rounded-full bg-emerald-400 ring-4 ring-[#10101c]" />
              <p class="text-gray-500 text-xs mb-0.5">Produk</p>
              <p class="text-white text-sm font-medium">{{ product.name }} — Lifetime License</p>
            </div>
            <!-- Total -->
            <div class="relative">
              <div class="absolute -left-[29px] top-1 w-3 h-3 rounded-full bg-emerald-300 ring-4 ring-[#10101c]" />
        ...
        <p class="text-gray-500 text-xs mb-6">
          License key & link download telah dikirim ke <span class="text-emerald-400">{{ buyerEmail }}</span>.
        </p>
