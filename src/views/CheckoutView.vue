...
function validate() {
  ...
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(appStore.payment.email)) e.email = 'Format email tidak valid'
...
<template>
  <div class="min-h-screen bg-[#060610] pb-20">
    ...
    <div class="relative overflow-hidden">
      <div class="absolute inset-0" style="background: linear-gradient(160deg, #022c22 0%, #064e3b 40%, #0f766e 100%)"></div>
      <div class="absolute inset-0 opacity-[.07]" style="background: repeating-linear-gradient(90deg, transparent, transparent 60px, rgba(255,255,255,.1) 60px, rgba(255,255,255,.1) 61px), repeating-linear-gradient(0deg, transparent, transparent 60px, rgba(255,255,255,.1) 60px, rgba(255,255,255,.1) 61px)"></div>
      <div class="relative z-10 text-center px-6 pt-28 pb-16">
        <span class="inline-flex items-center gap-2 bg-white/[.06] border border-white/[.08] text-emerald-300 text-[11px] font-bold px-4 py-1.5 rounded-full mb-5 tracking-widest uppercase backdrop-blur-sm">
          🔒 Checkout Aman
        </span>
    ...
    <div class="max-w-2xl mx-auto px-6 -mt-7 relative z-20">
      <div class="bg-[#10101c]/90 backdrop-blur-xl border border-white/[.06] rounded-2xl px-5 py-3.5 flex items-center justify-between shadow-xl shadow-black/40">
          ...
          >
            <div
              ...
              :class="s.n === currentStep
                ...
                : s.n < currentStep ? 'bg-emerald-500/20 text-emerald-400' : 'bg-white/5 text-slate-600'"
            >
              <span v-if="s.n < currentStep" class="text-sm">✓</span>
              <span v-else class="text-sm">{{ s.icon }}</span>
            </div>
            <span class="text-xs font-semibold hidden sm:block" :class="s.n === currentStep ? 'text-emerald-400' : 'text-slate-500'">{{ s.label }}</span>
          </button>
          <div v-if="idx < stepsMeta.length - 1" class="flex-1 mx-3">
            <div class="h-[2px] rounded-full transition-all duration-500" :class="s.n < currentStep ? 'bg-emerald-500' : 'bg-white/[.06]'"></div>
          </div>
        </template>
      </div>
    </div>
    ...
    <div class="max-w-2xl mx-auto px-6 mt-10">
      ...
      <div v-show="currentStep === 1" class="animate-fade-up">
        <div class="bg-[#10101c]/70 backdrop-blur-xl border border-white/[.06] rounded-3xl p-7 sm:p-9 shadow-2xl shadow-black/30">
          <div class="flex items-center gap-3.5 mb-7">
            <div class="w-11 h-11 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-teal-500/10 flex items-center justify-center text-xl border border-emerald-500/10">👤</div>
            <div>
          ...
          <div class="space-y-5">
            <div>
              ...
              <input v-model="appStore.payment.name" type="text" placeholder="Masukkan nama lengkap"
                ...
                :class="errors.name ? 'border-red-500/50' : 'border-white/[.06] focus:border-emerald-500 focus:shadow-[0_0_24px_rgba(16,185,129,.08)]'" />
              <p v-if="errors.name" class="text-red-400 text-xs mt-1.5 flex items-center gap-1"><span>⚠️</span> {{ errors.name }}</p>
            </div>
            <div class="grid sm:grid-cols-2 gap-5">
              <div>
                ...
                <input v-model="appStore.payment.email" type="email" placeholder="email@domain.com"
                  class="w-full bg-white/[.03] border-2 rounded-xl px-4 py-3.5 text-slate-100 placeholder-slate-600 outline-none transition-all duration-200"
                  :class="errors.email ? 'border-red-500/50' : 'border-white/[.06] focus:border-emerald-500 focus:shadow-[0_0_24px_rgba(16,185,129,.08)]'" />
                <p v-if="errors.email" class="text-red-400 text-xs mt-1.5 flex items-center gap-1"><span>⚠️</span> {{ errors.email }}</p>
              </div>
              <div>
                <label class="flex items-center gap-1.5 text-xs font-semibold text-slate-400 mb-2">
                  <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Nomor HP
                </label>
                <input v-model="appStore.payment.phone" type="tel" placeholder="08xxxxxxxxxx"
                  class="w-full bg-white/[.03] border-2 rounded-xl px-4 py-3.5 text-slate-100 placeholder-slate-600 outline-none transition-all duration-200"
                  :class="errors.phone ? 'border-red-500/50' : 'border-white/[.06] focus:border-emerald-500 focus:shadow-[0_0_24px_rgba(16,185,129,.08)]'" />
                <p v-if="errors.phone" class="text-red-400 text-xs mt-1.5 flex items-center gap-1"><span>⚠️</span> {{ errors.phone }}</p>
              </div>
            </div>
          </div>
        ...
        <div class="bg-[#10101c]/70 backdrop-blur-xl border border-white/[.06] rounded-3xl p-7 sm:p-9 shadow-2xl shadow-black/30">
          ...
          <div class="grid grid-cols-2 gap-3">
            <label v-for="m in paymentMethods" :key="m.value"
              ...
              :class="appStore.payment.method === m.value
              ...
              <input type="radio" v-model="appStore.payment.method" :value="m.value" class="hidden" />
              <span class="text-3xl group-hover:scale-110 transition-transform duration-200">{{ m.icon }}</span>
              ...
              <div v-if="appStore.payment.method === m.value"
        ...
        <div class="bg-[#10101c]/70 backdrop-blur-xl border border-white/[.06] rounded-3xl p-7 sm:p-9 shadow-2xl shadow-black/30">
          ...
          <div class="bg-white/[.02] rounded-2xl border border-white/[.06] overflow-hidden">
            <div class="flex items-center gap-4 p-5 border-b border-white/[.04]">
              <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shrink-0 ring-2 ring-emerald-500/20"
                style="background: linear-gradient(145deg, #022c22, #064e3b)">🖥️</div>
              <div class="flex-1 min-w-0">
                <span class="block text-sm font-bold text-white">{{ appStore.product.name }}</span>
                <span class="block text-[11px] text-slate-500 mt-0.5">{{ appStore.product.tagline }}</span>
              </div>
              <span class="text-base font-extrabold text-emerald-400 shrink-0">{{ formatPrice(appStore.product.price) }}</span>
            </div>
            <div class="p-5 border-b border-white/[.04] space-y-3">
              <div v-for="row in [
                ...
                { ico: '💳', lbl: 'Bayar via', val: paymentMethods.find(m => m.value === appStore.payment.method)?.label },
              ]" :key="row.lbl" class="flex items-center gap-3 text-sm">
            ...
            <div class="p-5 space-y-2.5">
              ...
              <div class="flex justify-between items-center pt-3 mt-1 border-t border-white/[.06]">
                ...
                <span class="text-2xl font-black text-emerald-400">{{ formatPrice(grandTotal) }}</span>
              </div>
            </div>
          </div>

          <div class="flex gap-3 bg-emerald-500/[.04] border border-emerald-500/10 rounded-xl p-4 mt-6">
            <span class="text-base mt-0.5 shrink-0">ℹ️</span>
            <p class="text-xs text-slate-400 leading-relaxed">
              License key dan link download dikirim ke <strong class="text-emerald-400">{{ appStore.payment.email }}</strong> dalam 5 menit setelah pembayaran.
            </p>
          </div>
          ...
          <div class="flex gap-3 mt-9">
            ...
            <button @click="handlePay" :disabled="isProcessing"
              ...
              :class="!isProcessing && 'hover:from-emerald-400 hover:to-teal-500 hover:-translate-y-0.5'">
      ...
      <div class="flex items-center justify-center gap-3 mt-8 flex-wrap">
        <span class="flex items-center gap-1.5 text-[10px] text-slate-600 bg-white/[.02] px-3 py-1.5 rounded-full border border-white/[.05]">🔒 SSL Encrypted</span>
        <span class="flex items-center gap-1.5 text-[10px] text-slate-600 bg-white/[.02] px-3 py-1.5 rounded-full border border-white/[.05]">🛡️ 30 Hari Garansi</span>
        <span class="flex items-center gap-1.5 text-[10px] text-slate-600 bg-white/[.02] px-3 py-1.5 rounded-full border border-white/[.05]">⚡ Instant Delivery</span>
      </div>
    </div>
  </div>
</template>
